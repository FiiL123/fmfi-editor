...
export default class Room {
    constructor(attributes, geometryType, geometry) {
        ...
        if (attributes.size>0){
            this.color = (purpose && purpose.fields.colour!== "") ? "rgb("+purpose.fields.colour+")" : "lightgray";
    ...
    createGeometry(geometryType,geometry){
        ...
        switch (geometryType) {
            case "rectangle":
                obj = new Rectangle(this,geometry.x,geometry.y,geometry.w,geometry.h,this.color);
                break;
            case "polygon":
    ...
    updateText(){
        this.numberText.text((this.attributes.has('custom-map-label')) ?
                this.attributes.get('custom-map-label') : this.number);
        const textPosition = this.geometry.getLabelPoint()
